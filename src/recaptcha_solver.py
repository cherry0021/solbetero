...
import time
import os
from utils import sleep_random, load_proxy, get_enumproxy
...
class RecaptchaSolver:
    ...
    def solve(self):
        ...
        # retry_limit = 15  # Maximum number of retries
        # retry_count = 0  # Counter for retries
        while True:
            # retry_count += 1
            # if retry_count > retry_limit:
            #     raise Exception("Exceeded maximum retry limit")
            # try:
            #     self.check_challenge_type()
            #     break  # If successful, exit the loop
            # except:
            #     print("Failed to check challenge type. Retrying...")
            #     time.sleep(2)  # W
            self.check_challenge_type()
    ...
    def switch_to_recap_iframe(self):
        ...
        recaptcha_iframe = WebDriverWait(self.driver, 20).until(EC.presence_of_element_located((By.CSS_SELECTOR, 'iframe[title="reCAPTCHA"]')))
    ...
    def trigger_captcha(self):
        ...
        recaptcha_checkbox = WebDriverWait(self.driver, 20).until(EC.presence_of_element_located((By.CLASS_NAME, "recaptcha-checkbox")))
    ...
    def switch_to_challenge_iframe(self):
        ...
        try:
            recaptcha_challenge_iframe = WebDriverWait(self.driver, 20).until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'iframe[title="recaptcha challenge expires in two minutes"]')))
        # recaptcha_challenge_iframe = WebDriverWait(self.driver, 25).until(EC.presence_of_element_located((By.CSS_SELECTOR, 'iframe[src*="api2/bframe"]')))
            ...
            recaptcha_challenge_iframe = WebDriverWait(self.driver, 20).until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'iframe[title="recaptcha challenge"]')))
            logger.debug("Found challenge iframe, switching to it...")
            self.driver.switch_to.frame(recaptcha_challenge_iframe.get_attribute("name"))
